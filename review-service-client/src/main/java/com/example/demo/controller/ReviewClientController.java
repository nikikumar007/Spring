...
public class ReviewClientController {
	...
	public String getReviewsFallBack(String name) {
		return "{'rating':5.0,'reviewerName':'Guest','location':'anonymous'}";
	}
	...
	@HystrixCommand(
			fallbackMethod = "getLocationFallBack") 
	
	public String getTopReviewerByLocation(@PathVariable("location") String location) {
		...
		String reqURL = baseURL+"/reviews/top/"+location;

		return this.template.getForObject(reqURL, String.class);
	}

	public String getLocationFallBack(String location) {
		return "Reviewer Not Present";
	}
